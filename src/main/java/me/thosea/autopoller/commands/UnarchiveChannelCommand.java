...
public final class UnarchiveChannelCommand extends DeferredCommandHandler {
	private final Category targetCategory = BOT.getOrThrow("ticket category", () -> {
		return BOT.guild.getCategoryById(CONFIG.ticketsCategoryId);
	});
	...
	protected SlashCommandData makeCommandData() {
		return Commands.slash("unarchivechannel", MSG.descUnarchiveChannel);
	}
	...
	protected boolean preDefer(Member member, SlashCommandInteraction event) {
		...
		if(category == CONFIG.ticketsCategoryId) {
			event.reply(MSG.unarchiveNotArchived).setEphemeral(true).queue();
			return false;
		} else if(category != CONFIG.archivedTicketsCategoryId) {
	...
	protected void handleDeferred(Member member, CommandInteractionPayload cmd, InteractionHook hook) {
		...
		if(ChannelUtils.getCategoryId(channel) != CONFIG.archivedTicketsCategoryId) {
		...
		((ICategorizableChannel) channel).getManager().setParent(targetCategory).queue(
				_ -> {
					hook.editOriginal(MSG.unarchiveSuccess.formatted(member.getAsMention()))
							.setAllowedMentions(List.of())
							.queue();
				},
				e -> ErrorReporter.deferredError(member, hook, "channel unarchive", e)
		);
	}
}